...
<aside :class="{'translate-x-0': sidebar.open, 'ltr:-translate-x-full rtl:translate-x-full': ! sidebar.open}" {{ $attributes->class(['fixed inset-y-0 ltr:left-0 rtl:right-0 z-50 w-64 flex-wrap items-center justify-between block p-0 my-4 overflow-y-auto transition-all duration-200 ease-in-out  bg-white border-0 shadow-none ltr:xl:ml-6 rtl:xl:mr-6 dark:bg-gray-800 z-990 max-w-64 rounded-2xl ltr:xl:translate-x-0 rtl:xl:translate-x-0']) }}>
    ...
    <div class="items-center block w-full h-auto grow basis-full" id="sidenav-collapse-main">
        <nav class="space-y-3">
            ...
            {{-- Posts --}}
            @can('read posts')
                <Link href="{{ route('dashboard.posts.index') }}" class="{{ request()->routeIs('dashboard.posts.index') ? 'bg-indigo-50 dark:bg-indigo-400 text-gray-700 dark:text-white font-semibold' : '' }} text-sm py-3 my-0 mx-2 flex items-center whitespace-nowrap px-4 font-medium text-gray-500 dark:text-white hover:bg-indigo-50 dark:hover:bg-indigo-400 rounded-lg shadow-none transition-colors ease-in-out">
                    <i class="fa-solid fa-pencil-alt"></i>
                    <span class="mx-2 text-sm">@lang('main.posts')</span>
                </Link>
            @endcan

            {{-- Categories --}}
            @can('read categories')
                <Link href="{{ route('dashboard.categories.index') }}" class="{{ request()->routeIs('dashboard.categories.index') ? 'bg-indigo-50 dark:bg-indigo-400 text-gray-700 dark:text-white font-semibold' : '' }} text-sm py-3 my-0 mx-2 flex items-center whitespace-nowrap px-4 font-medium text-gray-500 dark:text-white hover:bg-indigo-50 dark:hover:bg-indigo-400 rounded-lg shadow-none transition-colors ease-in-out">
                    <i class="fa-solid fa-folder"></i>
                    <span class="mx-2 text-sm">@lang('main.categories')</span>
                </Link>
            @endcan

            {{-- Tags --}}
            @can('read tags')
                <Link href="{{ route('dashboard.tags.index') }}" class="{{ request()->routeIs('dashboard.tags.index') ? 'bg-indigo-50 dark:bg-indigo-400 text-gray-700 dark:text-white font-semibold' : '' }} text-sm py-3 my-0 mx-2 flex items-center whitespace-nowrap px-4 font-medium text-gray-500 dark:text-white hover:bg-indigo-50 dark:hover:bg-indigo-400 rounded-lg shadow-none transition-colors ease-in-out">
                    <i class="fa-solid fa-tags"></i>
                    <span class="mx-2 text-sm">@lang('main.tags')</span>
                </Link>
            @endcan
